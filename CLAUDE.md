...
- **disk.rs** — iPod detection heuristics and bridge to the `crate-disk-helper` Swift binary. The Swift helper uses `DiskArbitration.framework` for typed disk properties and native mount/unmount (no sudo required). Built by `build.rs`, bundled as a Tauri sidecar.
...
- **macOS only:** Disk operations rely on macOS's `DiskArbitration.framework` via the bundled `crate-disk-helper` Swift binary. Not portable to other OSes.
