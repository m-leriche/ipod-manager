...
export const AlbumArtManager = () => {
  ...
  const selectAll = () => setSelected(new Set(albums.filter((a) => !a.has_cover_file).map((a) => a.folder_path)));
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center max-w-sm">
          <p className="text-text-tertiary text-xs mb-4">Choose a music folder to scan for missing album art</p>
          <div className="mb-4">
            <FolderPicker label="Folder" path={scanPath} onBrowse={browse} />
          </div>
          <button
            onClick={() => scan()}
            disabled={!scanPath}
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto bg-bg-secondary border border-border rounded-2xl min-h-0">
        {albums.length === 0 ? (
          <div className="py-12 text-center text-text-tertiary text-xs">No albums found in this folder</div>
        ) : missing.length === 0 && extractable.length === 0 ? (
          <div className="py-12 text-center text-text-tertiary text-xs">All albums have cover art!</div>
        ) : (
          <div>
            ...
            {hasCover.length > 0 && <AlbumGroup title="Has Cover Art" albums={hasCover} />}
          </div>
        )}
      </div>
    </>
