...
export const AlbumGroup = ({ title, subtitle, albums, selected, onToggle }: Props) => {
  ...
  return (
    <div>
      <div className="px-4 py-2.5 bg-bg-card border-b border-border sticky top-0 z-10">
        <span className="text-[11px] font-medium text-text-tertiary uppercase tracking-widest">{title}</span>
        {subtitle && <span className="text-[11px] text-text-tertiary ml-2">— {subtitle}</span>}
        <span className="text-[11px] text-text-tertiary ml-2">({albums.length})</span>
      </div>
      {albums.map((a) => (
        <div
          key={a.folder_path}
          className={`flex items-center gap-3 px-4 py-2.5 border-b border-border-subtle transition-colors ${
