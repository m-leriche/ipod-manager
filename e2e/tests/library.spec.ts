...
test.describe("Library — with data", () => {
  test.beforeEach(async ({ page, tauriMocks }) => {
    await tauriMocks.override({
      ...
      get_library_browser_data_paginated: MOCK_PAGINATED_BROWSER_DATA,
  ...
  test("column browser filters by genre", async ({ page, tauriMocks }) => {
    ...
    const filteredTracks = MOCK_TRACKS.filter((t) => t.genre === "Rock");
    const filteredPaginated = {
      tracks: { tracks: filteredTracks, total_count: filteredTracks.length, offset: 0, limit: 500 },
    ...
    await tauriMocks.setResponses({ get_library_browser_data_paginated: filteredPaginated });
  ...
  test("search input filters tracks", async ({ page, tauriMocks }) => {
    const filteredTracks = MOCK_TRACKS.filter((t) => t.title?.toLowerCase().includes("first"));
    const filteredPaginated = {
      tracks: { tracks: filteredTracks, total_count: filteredTracks.length, offset: 0, limit: 500 },
      genres: MOCK_BROWSER_DATA.genres,
