...
pub fn reorganize_library_file(
    ...
    // Try to update the existing record in place (preserving its id).
    ...
    if rows_changed == 0 {
        // Record wasn't found at old path — insert at the new location instead
        upsert_track(conn, &track_data, mtime, now)?;
    }

    // Always clean up any stale record at the old path.  When the UPDATE
    // matched, the record has already moved to new_path and this is a no-op.
    // When it didn't (e.g. the watcher re-created the record under a
    // slightly different path string), this removes the ghost.
    conn.execute(
